    ...
    private function executeAction(ComponentInterface $component, array $data): mixed
    {
        ...
        try {
            ...
            if ($e instanceof AjaxDispatcherException && preg_match('/non trouv(e|é)e|not found|introuvable/iu', $e->getMessage())) {
                ...
                // Keep action resolution strategy client-side (parent then child chain) and avoid mutating
                // another component server-side with states from the wrong scope. Return 200 to prevent a
                // noisy network error while preserving the `action_not_found` code for JS retry logic.
                DevError::respond($debugMsg, 200, 'action_not_found');
            }

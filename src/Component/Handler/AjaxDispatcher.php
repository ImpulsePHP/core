...
final class AjaxDispatcher
{
    ...
    private function executeAction(ComponentInterface &$component, array $data): ?bool
    {
        ...
        try {
            ...
            // If not found on the resolved component, attempt a fallback search among components listed in the request
            if (!$actionCalled) {
                try {
                ...
                foreach ($otherComponents as $otherId => $otherComponent) {
                    // skip the component we already tried
                    if ($otherComponent === $component) {
                        continue;
                    }

                    // prepare the other component (boot)
                    if (method_exists($otherComponent, 'boot')) {
                        App::container()->call([$otherComponent, 'boot']);
                    }

                    // apply provided states to the other component if any
                    if (isset($data['states']) && is_array($data['states'])) {
                        foreach ($data['states'] as $name => $value) {
                    ...
                    // try to find the method on this other component (same logic as above)
                    if (method_exists($otherComponent, $method)) {
                        ...
                        if (
                            ...
                            $actionCalled = true;
                            // replace the component reference so the caller will render the correct component
                    ...
                    if (!$actionCalled && $methods->exists($method)) {
                        ...
                        $requiredParams = $ref->getNumberOfRequiredParameters();
                        if (count($args) < $requiredParams) {
                            // not suitable, skip this component
                            continue;
                        }

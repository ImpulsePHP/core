    ...
    private function executeAction(ComponentInterface &$component, array $data): ?bool
    {
        ...
        try {
            ...
            if ($e instanceof AjaxDispatcherException && preg_match('/non trouv(e|é)e|not found|introuvable/iu', $e->getMessage())) {
                ...
                // Attempt a fallback: try the other components included in the request (client provides
                // the ordered list via the X-Impulse-Components header). This avoids returning a hard
                // 400 when the action actually exists on a different component (child) within the same
                // request context.
                try {
                    $otherComponents = ComponentResolver::allByAjax();
                } catch (\Throwable $ex) {
                    // If we cannot obtain the list, keep the original debug response.
                    DevError::respond($debugMsg, 400, 'action_not_found');
                }

                foreach ($otherComponents as $otherId => $otherComponent) {
                    // Skip the component we already attempted
                    if ($otherComponent === $component) {
                        continue;
                    }

                    // Ensure boot is called (to run setup/boot lifecycle)
                    if (method_exists($otherComponent, 'boot')) {
                        try {
                            App::container()->call([$otherComponent, 'boot']);
                        } catch (\Throwable) {
                            // ignore boot errors for fallback attempt
                        }
                    }

                    // Apply incoming states to the candidate component (same logic as resolveComponent)
                    if (isset($data['states']) && is_array($data['states'])) {
                        foreach ($data['states'] as $name => $value) {
                            try {
                                if (is_string($value) && StateCollection::shouldConvertValue($otherComponent, $name, $value)) {
                                    if (Helper::isValidateJson($value)) {
                                        $decoded = json_decode($value, true, 512, JSON_THROW_ON_ERROR);
                                        $value = is_array($decoded) ? $decoded : [$decoded];
                                    } else {
                                ...
                                $otherComponent->state($name, $value)->set($value);
                            } catch (\Throwable) {
                                // ignore per-state application errors during fallback
                            }
                        }
                    }

                    // Debug: log whether the candidate component has a 'rules' state (helps diagnose validation regressions)
                    try {
                        $candidateStates = method_exists($otherComponent, 'getStates') ? $otherComponent->getStates() : [];
                        $hasRules = array_key_exists('rules', $candidateStates);
                        error_log('[Impulse][AjaxDispatcher] Fallback candidate ' . $otherId . ' has_rules=' . ($hasRules ? '1' : '0'));
                    } catch (\Throwable) {
                        // ignore logging errors
                    }

                    // Attempt to call the method on the other component
                    try {
                        if (method_exists($otherComponent, $method)) {
                            $refMethod = new \ReflectionMethod($otherComponent, $method);
                            if (
                                $refMethod->getAttributes(Action::class)
                                && $refMethod->isPublic()
                                && !str_starts_with($method, '__')
                                ...
                                $actionResult = call_user_func_array([$otherComponent, $method], $args);
                                // Replace the component so rendering/updates target the right instance
                                $component = $otherComponent;
                                return $actionResult;
                            }
                        }

                        $methods = $otherComponent->getMethods();
                        if ($methods->exists($method)) {
                            ...
                            $requiredParams = $ref->getNumberOfRequiredParameters();
                            if (count($args) < $requiredParams) {
                                // not callable with provided args
                                continue;
                            }
                ...
                // If no other component handled the action, respond as before
                DevError::respond($debugMsg, 400, 'action_not_found');
            }
